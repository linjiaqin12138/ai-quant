...
def test_can_query_range_from_remote():
    ...
    res = binance.fetch_ohlcv(
        "BTC/USDT",
        "1h",
        june_30th_11_30_clock - timedelta(days=1),
        june_30th_11_30_clock,
    )
    assert len(res.data) == 24
...
def test_cn_market_exchange_fetch_history():
    ...
    stock_symbol = "515060"
    res = cn_market.fetch_ohlcv(
        symbol=stock_symbol,
        frame="1d",
        start=datetime(2024, 11, 1),
        end=datetime(2024, 11, 18, 1, 0, 0),
    )
    ...
    try:
        ...
        assert (
            str(err)
            == "结束时间(2024-11-01 00:00:00)必须大于开始时间(2024-11-01 00:00:00)"
        )
