...
def test_group_by_news_by_time():
    cases = {"jin10": {"from_h": 5, "to_h": 0}, "cointime": {"from_h": 8, "to_h": 0}}
    for platform in cases:
        context = cases[platform]
        context["news_list"] = news.get_news_during(
            platform, hours_ago(context["from_h"]), hours_ago(context["to_h"])
        )
        print(
            len(context["news_list"]),
            context["news_list"][0].timestamp,
            context["news_list"][-1].timestamp,
            context["news_list"][-1].timestamp - context["news_list"][0].timestamp,
        )
...
def test_group_by_news_by_platform():
    ...
    print(
        render_news_in_markdown_group_by_platform(
            {"sina": sina_news, "qq-news": qq_news}
        )
    )
