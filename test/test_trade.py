...
def test_only_call_once_when_parallel_call():
    ...
    def fetch_ohlcv_side_effect(symbol, frame, start, end, *args, **kwargs):
        ...
        while current < end:
            data.append(
                Ohlcv(
                    timestamp=current, open=100, high=110, low=90, close=105, volume=1
                )
            )
    ...
    def fetch_data():
        ...
        crypto_module.get_ohlcv_history(
            "BTC/USDT", "1h", datetime.now() - timedelta(days=1), datetime.now()
        )
...
def generate_mock_ohlcv_data(
    start_dt: datetime, end_dt: datetime, frame: str = "1h"
) -> OhlcvHistory:
    ...
    current_dt = (
        ...
        if frame == "1h"
        else current_dt.replace(second=0, microsecond=0)
    )

    while current_dt < end_dt:
        data.append(
            Ohlcv(timestamp=current_dt, open=100, high=110, low=90, close=105, volume=1)
        )
        current_dt += delta
...
def test_can_query_range_from_remote_and_second_time_hit_cache():
    ...
    crypto = CryptoTrade(exchange=mock_exchange)

    june_30th_11_clock = datetime(2024, 6, 30, 11, 56, 40, 290935)

    # 第一次查询会去后端
    result = crypto.get_ohlcv_history(
        "BTC/USDT", "1h", june_30th_11_clock - timedelta(days=1), june_30th_11_clock
    )
    ...
    result = crypto.get_ohlcv_history(
        "BTC/USDT",
        "1h",
        june_30th_11_clock - timedelta(hours=20),
        june_30th_11_clock - timedelta(hours=10),
    )
    ...
    result = crypto.get_ohlcv_history(
        "BTC/USDT", "1h", june_30th_11_clock - timedelta(days=2), june_30th_11_clock
    )
    ...
    result = crypto.get_ohlcv_history(
        "BTC/USDT",
        "1h",
        june_30th_11_clock - timedelta(days=4),
        june_30th_11_clock - timedelta(days=3),
    )
    ...
    result = crypto.get_ohlcv_history(
        "BTC/USDT",
        "1h",
        june_30th_11_clock - timedelta(days=5),
        june_30th_11_clock - timedelta(days=4, hours=23),
    )
    ...
    result = crypto.get_ohlcv_history(
        "BTC/USDT",
        "1h",
        june_30th_11_clock - timedelta(days=6),
        june_30th_11_clock + timedelta(hours=4),
    )
