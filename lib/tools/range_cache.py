...
GetCacheDataFunc = Callable[[DbTransaction, datetime, datetime], List[T]] # Function to get actual data items from cache storage
StoreDataFunc = Callable[[DbTransaction, List[T]], None] # Function to store actual data items
# Removed: KeyGeneratorFunc = Callable[[Callable, tuple, dict], str]

def _generate_range_cache_key(func: Callable, bound_args: inspect.BoundArguments, key_param_names: List[str]) -> str:
    """
    ...
    try:
        ...
        for name in sorted(key_param_names): # Sort names for consistency
            if name in arguments:
                ...
                logger.warning(f"Key parameter '{name}' not found in arguments for {func.__qualname__}. Skipping.")
                key_parts.append(f"{name}=<NOT_FOUND>") # Indicate missing param in key

        return "||".join(key_parts)
    except Exception as e:
        logger.error(f"Error generating range cache key for {func.__name__}: {e}", exc_info=True)
    ...
    def decorator(func: Callable[..., List[T]]) -> Callable[..., List[T]]:
        ...
        for name in key_param_names:
            if name not in valid_params:
                 raise TypeError(f"key_param_name '{name}' is not a valid parameter of function {func.__name__} (excluding start/end)")
        if 'start' not in sig.parameters or 'end' not in sig.parameters:
            raise TypeError(f"Decorated function {func.__name__} must accept 'start' and 'end' arguments.")
        if sig.parameters['start'].annotation != datetime or sig.parameters['end'].annotation != datetime:
             logger.warning(f"Parameters 'start' and 'end' in {func.__name__} should preferably be annotated with datetime.")
        ...
        def wrapper(*args, **kwargs) -> List[T]:
            ...
            base_key = _generate_range_cache_key(func, bound_args, key_param_names) # Use the new helper
            metadata_key = f"{base_key}{metadata_key_suffix}"
            lock_key = f"{base_key}{lock_key_suffix}" # Generate lock key
            logger.debug(f"Range cache check. Base key: {base_key}, Metadata key: {metadata_key}, Lock key: {lock_key}, Range: [{start}, {end})")
            ...
            try:
                with create_transaction() as db:
                    ...
                    if metadata and \
                    metadata.get('query_range') and \
                    metadata['query_range'][0] <= start_ts and \
                    end_ts <= metadata['query_range'][1]:
                        logger.info(f"Range cache HIT (Full - outside lock check): {base_key} for range [{start}, {end})")
                        return get_data_by_cache(db, start, end)
            except Exception as e:
                 logger.warning(f"Failed to read metadata outside lock for {metadata_key}: {e}. Proceeding to lock.", exc_info=True)

            logger.info(f"Range cache MISS or PARTIAL HIT or read failure: {base_key}. Acquiring lock for potential fetch.")

            @with_lock(lock_key, max_concurrent_access=1, timeout=300, expiration_time=300)
            def fetch_and_cache_with_lock():
                ...
                with create_transaction() as db:
                    try:
                        ...
                        logger.error(f"Failed to read metadata *inside lock* for {metadata_key}: {e}. Assuming no cache.", exc_info=True)
                        metadata = None
                    ...
                    if metadata and \
                       ...
                       metadata['query_range'][0] <= start_ts and \
                       end_ts <= metadata['query_range'][1]:
                        logger.info(f"Range cache HIT (Full) *inside lock*: {base_key} for range [{start}, {end})")
                        # Pass db only if expected by the function
                        return get_data_by_cache(db, start, end)

                    logger.info(f"Range cache MISS or PARTIAL HIT *inside lock*: {base_key}. Proceeding with fetch.")

                    # 2. Determine range(s) to fetch based only on query_range
                    fetch_ranges: List[Tuple[datetime, datetime]] = []
                    current_metadata = metadata if metadata and 'query_range' in metadata else None
                    # Initialize new_metadata based on current request if no cache exists
                    new_metadata: CacheMetadata = current_metadata.copy() if current_metadata else {
                        'query_range': [start_ts, end_ts]
                    }
                    target_query_range_start = start_ts
                    target_query_range_end = end_ts

                    if current_metadata is None:
                        logger.info(f"No valid cache metadata found inside lock. Fetching initial range [{start}, {end}).")
                        fetch_ranges.append((start, end))
                        # new_metadata is already set correctly
                    else:
                        cached_query_start_ts, cached_query_end_ts = current_metadata['query_range']
                        ...
                        if start_ts < cached_query_start_ts:
                            ...
                            logger.info(f"Fetching data before cache: [{fetch_start}, {fetch_end})")
                        ...
                        if end_ts > cached_query_end_ts:
                            ...
                            fetch_end = end
                            logger.info(f"Fetching data after cache: [{fetch_start}, {fetch_end})")
                            fetch_ranges.append((fetch_start, fetch_end))

                        # Update the query range in metadata to reflect the target range
                        new_metadata['query_range'] = [int(target_query_range_start), int(target_query_range_end)] # Ensure int

                    for fr_start, fr_end in fetch_ranges:
                        logger.debug(f"Calling original function {func.__name__} for range [{fr_start}, {fr_end})")
                        try:
                            # 修改原始函数的start和end参数为当前范围
                            call_kwargs = bound_args.arguments.copy()
                            call_kwargs['start'] = fr_start
                            call_kwargs['end'] = fr_end
                            current_new_data: List[T] = func(**call_kwargs) # Call the original decorated function

                            if current_new_data:
                                logger.debug(f"Storing {len(current_new_data)} new items fetched for range [{fr_start}, {fr_end}).")
                                store_data(db, current_new_data) # Store fetched data
                            else:
                                logger.debug(f"No data returned by original function for range [{fr_start}, {fr_end}).")

                        except Exception as e:
                            logger.error(f"Error calling original function {func.__name__} or storing data for range [{fr_start}, {fr_end}): {e}", exc_info=True)
                    ...
                    logger.info(f"Updating metadata for key {metadata_key} to {new_metadata} (inside lock)")
                    ...
                    logger.debug(f"Retrieving final data for range [{start}, {end}) from cache via get_data_by_cache (inside lock).")
                    return get_data_by_cache(db, start, end)
