...
def generate_cache_key(func: Callable, args: tuple, kwargs: dict) -> str:
    ...
    try:
        ...
        bound_args = sig.bind_partial(*args, **kwargs) # Use bind_partial before apply_defaults
        ...
        for k, v in sorted(bound_args.arguments.items()):
            ...
            try:
                # 对值进行JSON序列化以获得稳定表示
                # sort_keys确保字典顺序一致
                arg_repr = json.dumps(v, sort_keys=True, default=str) # default=str处理datetime等
            except TypeError:
                # 如果JSON序列化失败，回退到repr
                arg_repr = repr(v)
        ...
        logger.error(f"Error generating cache key for {func.__name__}: {e}", exc_info=True)
        # 回退到不太可靠但总能工作的键
    ...
    def decorator(func: Callable[P, R]) -> Callable[P, R]:
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
                        ...
                        if cached_dict is not None:
                            try:
                                ...
                                if expire_time_iso and data_serialized:
                                    expire_time = datetime.fromisoformat(expire_time_iso)
                                    if expire_time > now:
                                        logger.debug(f"Cache HIT (DB): {db_cache_key}")
                                        result = deserializer(data_serialized) if deserializer else data_serialized
                                        # 更新内存缓存
                                        memory_cache[cache_key] = (result, expire_time)
                                        return result
                                    else:
                                        logger.debug(f"Cache EXPIRED (DB): {db_cache_key}")
                                        db.kv_store.delete(db_cache_key)
                                        db.commit()
                                else:
                                     logger.warning(f"Cache CORRUPT (DB - missing fields): {db_cache_key}")

                            except (json.JSONDecodeError, TypeError, ValueError) as e:
                                 logger.error(f"Cache ERROR (DB Deserialize): {db_cache_key} - {e}", exc_info=True)
                                 # 记录错误，视为缓存未命中
                except Exception as db_err:
                    logger.error(f"Database error during cache check for {db_cache_key}: {db_err}", exc_info=True)
            ...
            try:
                 ...
                 logger.error(f"Error executing decorated function {func.__name__}: {func_exc}", exc_info=True)
            ...
            if use_db_cache:
                 try:
                    ...
                    logger.error(f"Cache ERROR (DB Serialize/Set): {db_cache_key} - {e}", exc_info=True)
                    # 记录错误，但继续而不更新数据库缓存
