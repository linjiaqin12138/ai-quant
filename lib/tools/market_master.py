...
def format_indicators(ohlcv_list: List[Ohlcv], use_indicators: SupportIndicators, max_length=20) -> str:
    ...
    if 'sma' in use_indicators:
        ...
        if len(ohlcv_list) >= 20:
            ...
            result_texts.append(f"- 过去{len(sma20)}天20日简单移动平均线 (SMA5): {sma20}")
...
def validate_crypto_advice(advice: str, max_cost: float, max_sell_amount: float) -> Dict:
    try:
        ...
        if advice_json['action'] == 'buy':
            ...
            assert isinstance(advice_json['buy_cost'], (int, float)), "'buy_cost'字段必须是浮点数类型"
            assert advice_json['buy_cost'] > 0, "'buy_cost'必须大于0"
            assert advice_json['buy_cost'] <= max_cost, f"买入金额{advice_json['buy_cost']}超过可用余额{max_cost}"
        elif advice_json['action'] == 'sell':   
            assert 'sell_amount' in advice_json, "卖出操作缺少'sell_amount'字段"
            assert isinstance(advice_json['sell_amount'], (int, float)), "'sell_amount'字段必须是浮点数类型"
...
class MarketMaster:
    ...
    def give_trade_adevice(self, ctx: TradeContext) -> AgentAdvice:
        return self.give_crypto_trade_advice(ctx) if 'USDT' in ctx.symbol else self.give_ashare_trade_advice(ctx)
    ...
    def give_crypto_trade_advice(self, ctx: TradeContext) -> AgentAdvice:
        ...
        if not ctx.ohlcv_list:
            ctx.ohlcv_list = crypto.get_ohlcv_history(ctx.symbol, frame='1d', limit=65).data
        ...
        future_info_text = format_binance_future_info(
            global_long_short_account=self.binance_exchange.get_u_base_global_long_short_account_ratio(future_symbol, '15m', hours_ago(1))[-1]['longShortRatio'],
            top_long_short_account=self.binance_exchange.get_u_base_top_long_short_account_ratio(future_symbol, '15m', hours_ago(1))[-1]['longShortRatio'],
            top_long_short_amount=self.binance_exchange.get_u_base_top_long_short_ratio(future_symbol, '15m', hours_ago(1))[-1]['longShortRatio'],
            future_rate=self.binance_exchange.get_latest_futures_price_info(future_symbol)['lastFundingRate']
        ) if self.use_crypto_future_info else ''
        ...
        llm_ask = get_llm_tool(system_prompt, self.llm_provider, self.model, temperature=self.temperature, response_format='json')

        @with_retry((JsonReplyError), API_MAX_RETRY_TIMES)
        def retryable():
            rsp = validate_crypto_advice(llm_ask(user_prompt), ctx.account_info['free'], ctx.account_info['hold_amount'])
    ...
    def give_ashare_trade_advice(self, ctx: TradeContext) -> AgentAdvice:
        if ctx.ohlcv_list is None:
            ctx.ohlcv_list = ashare.get_ohlcv_history(ctx.symbol, frame='1d', limit=65).data
        stock_info = get_ashare_stock_info(ctx.symbol)
        ohlcv_text = format_ohlcv_list(ctx.ohlcv_list[-30:])
        detected_patterns_text = format_ohlcv_pattern(ctx.ohlcv_list) if self.detect_ohlcv_pattern else ''
        indicators_text = format_indicators(ctx.ohlcv_list, self.use_indicators)
