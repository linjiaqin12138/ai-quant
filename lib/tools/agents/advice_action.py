...
USER_PROMPT_TEMPLATE = """ 分析以下加密货币的信息，并给出交易建议：
...
AccountInfo = Dict[Literal['free', 'hold_amount', 'hold_val'], float]
...
def format_indicators(ohlcv_list: List[Ohlcv], use_indicators: SupportIndicators, max_length=20) -> str:
    """
    计算并格式化指定的技术指标
    ...
    if 'sma' in use_indicators:
        ...
        result_texts.append(f"- 过去{len(sma20)}天5日简单移动平均线 (SMA5): {sma20}")
    if 'rsi' in use_indicators:
        ...
        result_texts.append(f"- 过去{len(rsi_values_rounded)}天相对强弱指数 (RSI): {rsi_values_rounded}")
    if 'boll' in use_indicators:
    ...
    if 'macd' in use_indicators:
        ...
        macd_hist = map_by(macd.macdhist[-max_length:], round_to_5)  # 假设macdhist存储MACD柱状图数据
    ...
    if 'stoch' in use_indicators:
        ...
        result_texts.append(f"- 过去{len(stoch_slowd)}天随机指标 (Stochastic Oscillator):")
        result_texts.append(f"    - %K: {stoch_slowk}")
        result_texts.append(f"    - %D: {stoch_slowd}")
    if 'atr' in use_indicators:
        atr_values = atr_indicator(ohlcv_list).atr[-max_length:]
        atr_values_rounded = map_by(atr_values, round_to_5)
        result_texts.append(f"- 过去{len(atr_values_rounded)}天平均真实波幅 (ATR): {atr_values_rounded}")
            
    return '\n'.join(result_texts)

def format_crypto_account_info(account_info: CryptoExchangeFutureInfo) -> str:
    free = account_info['free']
    hold_amount = account_info['hold_amount']
    hold_val = round_to_5(account_info['hold_val'])
    return f"""- USDT余额: {free}
...
def format_crypto_history(history: TradeHistoryList) -> str:
    def format_trade_record(trade: TradeLog):
        ...
        if action == 'buy':
            return f'- {timestamp} 花费{round_to_5(cost)}USDT买入{round_to_5(amount)}份, 仓位{round_to_5(position_ratio)}%, 理由：{summary}'
        else:
            return f'- {timestamp} 卖出{round_to_5(amount)}份, 仓位{round_to_5(position_ratio)}%, 理由：{summary}'
    return "\n".join(format_trade_record(trade) for trade in history) if history else "暂无交易历史"
...
def format_binance_future_info(
    ...
    return '\n'.join([
        f'- 多空持仓人数比：{global_long_short_account}',
        f'- 大户账户数多空比: {top_long_short_account}',
        f'- 大户持仓量多空比: {top_long_short_amount}',
        f'- 资金费率：{future_rate}'
    ])

def consutruct_crypto_user_prompt(
    coin_name: str,
    ohlcv: str,
    indicators: str,
    pattern: str,
...
def validate_crypto_advice(advice: str, max_cost: float, max_sell_amount: float) -> Dict:
    try:
        ...
        if advice_json['action'] == 'buy':
            ...
            if advice_json['amount'] > max_sell_amount:
                advice_json['amount'] = max_sell_amount
        return advice_json
    except Exception as err:
        raise JsonReplyError(err)
    
def advice_crypto_action(
    coin_name: str,
    ohlcv_list: List[Ohlcv],
    account_info: AccountInfo,
    trade_history: TradeHistoryList,
    exchange_future_info: CryptoExchangeFutureInfo = None,
    related_news: str = '',
    risk_prefer: str = '风险厌恶型',
    strategy_prefer: str = '中长期投资',
    use_indicators: SupportIndicators = ['sma', 'rsi', 'boll', 'macd', 'stoch', 'atr'],
    ...
    ohlcv_text = format_ohlcv_list(ohlcv_list[-30:])
    if detect_ohlcv_pattern:
        detected_patterns_text = format_ohlcv_pattern(ohlcv_list)
    indicators_text = format_indicators(ohlcv_list, use_indicators)
    account_info_text = format_crypto_account_info(account_info)
    history_text = format_crypto_history(trade_history[-10:])
    future_info_text = ''
    if exchange_future_info:
        future_info_text = format_binance_future_info(**exchange_future_info)
    system_prompt = CRYPTO_SYSTEM_PROMPT_TEMPLATE.format(
        coin_name = coin_name,
        risk_prefer = risk_prefer,
        strategy_prefer = strategy_prefer
    )
    user_prompt = consutruct_crypto_user_prompt(
        coin_name,
        ohlcv_text,
        indicators_text,
        detected_patterns_text,
    ...
    def retryable():
        ...
        rsp = validate_crypto_advice(llm.ask(user_prompt), account_info['free'], account_info['hold_amount'])
        # Filter rsp to keep only valid parameters
        valid_keys = {'action', 'reason', 'summary', 'cost', 'amount'}
...
def format_ashare_account_info(account_info: AccountInfo, max_lots_can_buy: int) -> str:
    free = account_info['free']
    hold_lots = account_info['hold_amount'] // 100
    hold_val = round_to_5(account_info['hold_val'])
    return f"""- 可用资金: {free}RMB (可买{max_lots_can_buy}手)
...
def format_ashare_history(history: TradeHistoryList) -> str:
    def format_trade_record(trade: TradeLog):
        ...
        timestamp = ts_to_dt(trade['timestamp']).strftime('%Y-%m-%d')
        cost = trade.get('cost')
        lots = trade.get('amount') // 100
        position_ratio = int(trade['position_ratio'] * 100)
        summary = trade['summary']
        if action == 'buy':
            return f'- {timestamp} 花费{round_to_5(cost)}RMB买入{lots}手, 仓位{round_to_5(position_ratio)}%, 理由：{summary}'
        else:
            return f'- {timestamp} 卖出{lots}手, 仓位{round_to_5(position_ratio)}%, 理由：{summary}'
    return "\n".join(format_trade_record(trade) for trade in history) if history else "暂无交易历史"
...
def advice_ashare_action(
    stock_name: str,
    curr_price: float,
    ohlcv_list: List[Ohlcv],
    account_info: AccountInfo,
    trade_history: List[TradeLog],
    related_news: str = '',
    risk_prefer: str = '风险厌恶型',
    strategy_prefer: str = '中长期投资',
    use_indicators: SupportIndicators = ['sma', 'rsi', 'boll', 'macd', 'stoch', 'atr'],
    ...
    if detect_ohlcv_pattern:
        detected_patterns_text = format_ohlcv_pattern(ohlcv_list)
    indicators_text = format_indicators(ohlcv_list, use_indicators)
    max_lots_can_buy = account_info['free'] // curr_price // 100
    account_info_text = format_ashare_account_info(account_info, max_lots_can_buy)
    history_text = format_ashare_history(trade_history[-10:])
    ...
    def retryable():
        ...
        filtered_rsp = {k: v for k, v in rsp.items() if k in valid_keys}
        if filtered_rsp['action'] == 'buy':
            filtered_rsp['cost'] = filtered_rsp['lots'] * 100 * curr_price
            del filtered_rsp['lots']
        elif filtered_rsp['action'] == 'sell':
            filtered_rsp['amount'] = filtered_rsp['lots'] * 100
            del filtered_rsp['lots']
        return AgentAdvice(**filtered_rsp)
...
__all__ = [
    'advice_crypto_action',
    'AgentAdvice',
    'CryptoExchangeFutureInfo',
    'TradeLog',
    'advice_ashare_action'
]
