...
def render_news_in_markdown_group_by_platform(
    news_list_per_platform: Dict[str, List[NewsInfo]]
) -> str:
    ...
    def platform_to_section(platform: str) -> str:
        if len(news_list_per_platform[platform]) > 0:
            news_with_title = filter_by(
                news_list_per_platform[platform], lambda n: n.title
            )
            news_without_title = filter_by(
                news_list_per_platform[platform], lambda n: not n.title
            )
    ...
    return "# 各大平台新闻\n" + "\n".join(
        map_by(news_list_per_platform.keys(), platform_to_section)
    )


def render_news_in_markdown_group_by_time_for_each_platform(
    news_list_per_platform: Dict[str, List[NewsInfo]]
) -> str:
