...
class MACDIndicatorResult:
    ...
    def is_gold_cross(self) -> bool:
        ...
        return not macdhist.empty and (macdhist.iloc[-1] > 0) and (macdhist.iloc[-2] < 0)
    ...
    def is_turn_good(self) -> bool:
        macdhist = self.macdhist_series.dropna()
        return len(macdhist) >= 3 and (macdhist.iloc[-1] - macdhist.iloc[-2] > 0) and (macdhist.iloc[-2] - macdhist.iloc[-3] < 0) and (macdhist.iloc[-1] < 0)
...
def bollinger_bands_indicator(ohlcv_list: List[Ohlcv], timeperiod: int = 20, nbdevup: float = 2.0, nbdevdn: float = 2.0) -> BollingerBandsIndicatorResult:
    """
    ...
    """
    df = to_df(ohlcv_list)
    upperband, middleband, lowerband = talib.BBANDS(df['close'], timeperiod=timeperiod, nbdevup=nbdevup, nbdevdn=nbdevdn)
    ...
    return BollingerBandsIndicatorResult(upperband=upperband_series.tolist(), middleband=middleband_series.tolist(), lowerband=lowerband_series.tolist())

def macd_indicator(ohlcv_list: List[Ohlcv], fastperiod: int = 12, slowperiod: int = 26, signalperiod: int = 9) -> MACDIndicatorResult:
    """
    ...
    return MACDIndicatorResult(macd=macd_series.tolist(), macdsignal=macdsignal_series.tolist(), macdhist=macdhist_series.tolist())

def stochastic_oscillator_indicator(ohlcv_list: List[Ohlcv], fastk_period: int = 14, slowk_period: int = 3, slowd_period: int = 3) -> StochasticOscillatorIndicatorResult:
    """
    ...
    return StochasticOscillatorIndicatorResult(slowk=slowk_series.tolist(), slowd=slowd_series.tolist())
