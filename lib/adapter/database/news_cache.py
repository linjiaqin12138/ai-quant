...
class HotNewsCache:
    ...
    def get_news_by_id(self, id: str) -> Union[NewsInfo, None]:
        compiled = (
            select(hot_news_cache).where(hot_news_cache.c.news_id == id).compile()
        )
    ...
    def get_news_by_time_range(
        ...
        compiled = (
            select(hot_news_cache)
            .where(*conditions)
            .order_by(hot_news_cache.c.timestamp.asc())
            .compile()
        )
        res = self.session.execute(compiled.string, compiled.params)
