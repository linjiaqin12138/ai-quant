...
class DbBasedDistributedLock(DistributedLock):
    ...
    def _check_lock_available_result(
        self, begined_session, for_update=True
    ) -> Union[Literal[-1, 0, 1], dict | None]:
        ...
        if lock_row:
            ...
            context_data["locks"] = [
                lock
                for lock in context_data.get("locks", [])
                if lock["expiration"] > time.time()
            ]
            return (
                1 if len(context_data["locks"]) < self.max_concurrent_access else 0
            ), context_data
        else:
            return -1, None
    ...
    def acquire(self):
        ...
        try:
            ...
            with session.begin():
                self._begin_immediate_transaction_if_sqlite(session)
                lock_resource_result, context_data = self._check_lock_available_result(
                    session
                )
                if lock_resource_result == 1:
                    ...
                    session.execute(
                        events.insert().values(
                            key=self.name, context=json.dumps(context_data), type="json"
                        )
                    )
                else:
            ...
            with session.begin():
                ...
                if lock_row:
                    ...
                    lock_to_remove = next(
                        (
                            lock
                            for lock in context_data["locks"]
                            if lock["id"] == lock_id
                        ),
                        None,
                    )
