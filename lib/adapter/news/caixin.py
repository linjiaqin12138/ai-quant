...
def query_latest_news_with_api(p_num: int, p_size: int):
    ...
    return sorted(filter_by(rspBody['data']['data'], is_valid_news), key=lambda x: x['time'], reverse=True)
...
def get_latest_news_of_caixin(start: datetime = days_ago(1)) -> List[NewsInfo]:
    ...
    while True:
        ...
        for news in news_data:
            ...
            logger.debug(f"{news_time_dt.isoformat()} {news['title']}, is in range: {news_time_dt >= start}, is top: {news.get('top')}")
