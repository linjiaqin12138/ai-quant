...
def get_news_of_jin10(
    ...
    def map_to_news_info(item: Dict) -> NewsInfo:
        ...
        return NewsInfo(
            ...
            url=item_data.get("source_link")
            or item_data.get("link")
            or item_data.get("pic")
            or "",
    ...
    while True:
        ...
        filtered_news_info_list = filter_by(
            news_info_list,
            lambda x: x.timestamp >= start
            and x.timestamp < end
            and (x.title or x.description),
        )
        map_by(
            filtered_news_info_list,
            lambda x: logger.debug(f"{x.timestamp}: {x.title or x.description}"),
        )
        result.extend(filtered_news_info_list)
        if news_info_list[0].timestamp < start:
            break
        end = news_info_list[0].timestamp - timedelta(
            seconds=1
        )  # prevent news duplicate
