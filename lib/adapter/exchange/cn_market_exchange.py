...
class AshareExchange(ExchangeAPI):
    ...
    def fetch_ohlcv(
        ...
        if rounded_end <= rounded_start:
            raise ValueError(
                f"结束时间({rounded_end})必须大于开始时间({rounded_start})"
            )
        ...
        is_after_utc_730_in_a_day = (
            frame == "1d"
            and end_in_utc.strftime("%Y-%m-%d") == end.strftime("%Y-%m-%d")
            and (
                end_in_utc.hour > 7 or (end_in_utc.hour == 7 and end_in_utc.minute > 29)
            )
        )
