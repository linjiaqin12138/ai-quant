...
class Worker:
    ...
    def init_executor(self):
        ...
        logger.info(
            f"已初始化{'进程' if self.use_process else '线程'}池，最大并发数: {self.max_workers}"
        )
    ...
    def _execute_task(self, task: Task) -> Any:
        ...
        while True:
            try:
                ...
                logger.error(
                    f"任务 {task.task_id} 执行失败: {str(e)} {traceback.format_exc()}"
                )

                if retry < task.retry_count:
                    retry += 1
                    logger.info(
                        f"将在 {task.retry_interval}秒 后重试任务 {task.task_id}，第 {retry}/{task.retry_count} 次重试"
                    )
