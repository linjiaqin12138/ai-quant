...
class OpenAiApiMixin:
    ...
    def _build_req_body(self, context: List, tools: Optional[List[Dict[str, Any]]] = None) -> str:
        result = remove_none({
            "model": self.model,
            "messages": context,
            "stream": False,
    ...
    def ask_with_tools(self, context: List, available_tools: Optional[List[str]] = None) -> Dict[str, Any]:
        """支持工具调用的请求方法"""
        # 获取可用工具
        tools = self.get_available_tools(available_tools) if hasattr(self, 'get_available_tools') else None
        
        json_body_str = self._build_req_body(context, tools)
        headers = self._build_req_header()

        logger.debug(f"{self.model} calling with tools data: {json_body_str}")
        logger.info(f"{self.model} calling with tools body size: {len(json_body_str)} Byte")
        response = requests.post(f"{self.endpoint}/v1/chat/completions", data=json_body_str, headers=headers, stream=False)
        logger.info(f"{self.model} calling with tools statusCode: {response.status_code}")
        logger.debug(f"{self.model} response header {response.headers}")
        logger.debug(f"{self.model} response body {response.text}")
