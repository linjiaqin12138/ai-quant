...
class G4f(OpenAiApiMixin, LlmAbstract):
    ...
    @with_retry(
        (OpenAiRetryableError, RateLimitError, ResponseError, ResponseStatusError),
        API_MAX_RETRY_TIMES,
    )
    def _ask(self) -> str:
        logger.debug(
            f"G4F calling data: {json.dumps(self.chat_context, ensure_ascii=False, indent=2)}"
        )
        logger.info(
            f"G4F API calling with body size: {len(json.dumps(self.chat_context))} Byte"
        )
