...
class BackTest:
    ...
    def save_recovery_data(self, df: pd.DataFrame, state: SimpleState, iter_from_idx: int, history: List[Ohlcv]):
    ...
    def save_results(self, df: pd.DataFrame, history: List[Ohlcv]):
        ...
        if 'macd' in self.show_indicators:
            ...
            add_plot.extend([
                mpf.make_addplot(macdhist_series.where(macdhist_series >= 0), type='bar', panel=2, color='g', alpha=1),
                mpf.make_addplot(macdhist_series.where(macdhist_series < 0), type='bar', panel=2, color='r', alpha=1)
            ])
        ...
        if 'boll' in self.show_indicators:
            ...
            add_plot.extend([
                mpf.make_addplot(boll.lowerband_series.iloc[-len(df):], color='red'),
                mpf.make_addplot(boll.middleband_series.iloc[-len(df):], color='gray'),
                mpf.make_addplot(boll.upperband_series.iloc[-len(df):], color='green')
            ])
    ...
    def run(self):
        if self.recovery_file and os.path.exists(self.recovery_file):
            df, start_time, end_time, self.strategy.state, iter_from_idx, history = self.load_recovery_data()
        else:
            iter_from_idx = 0
            ...
            trace_back_start = self.strategy._trace_back_business_day_from(
                self.strategy._data_fetch_amount, 
                start_time
            )
            ...
            self.strategy.state.set('bt_start_amount', self.strategy.investment / history[self.strategy._data_fetch_amount].open)
            self.strategy.state.set('bt_test_range', [dt_to_ts(start_time), dt_to_ts(self.end_time)])

        self.strategy._prepare()

        try:
            process_bar = tqdm(total=len(history) - self.strategy._data_fetch_amount, desc="Progress")
    ...
    def process_history_data(self, current_idx: int, history: List[Ohlcv], df: pd.DataFrame):
        self.strategy.state.set('bt_current_price', history[current_idx].open)
        self.strategy.state.set('bt_current_time', dt_to_ts(history[current_idx].timestamp))
        self.strategy.state.set('bt_observed_action', 'none')
        self.strategy.state.set('bt_addtional_info', {})

        # 运行策略
        self.strategy._core(history[current_idx - self.strategy._data_fetch_amount:current_idx])

        df.loc[history[current_idx].timestamp, 'strategy_gain'] = (
            self.strategy.hold_amount * self.strategy.current_price + self.strategy.free_money
        )
        ...
        if self.strategy.state.get('bt_observed_action') == 'buy':
            df.loc[history[current_idx].timestamp, 'buy_point'] = self.strategy.current_price 
        if self.strategy.state.get('bt_observed_action') == 'sell':
            df.loc[history[current_idx].timestamp, 'sell_point'] = self.strategy.current_price
        for col in self.strategy.state.get('bt_addtional_info'):
            df.loc[history[current_idx].timestamp, col] = self.strategy.state.get('bt_addtional_info')[col]