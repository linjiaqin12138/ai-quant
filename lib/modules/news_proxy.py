...
class NewsFetchProxy(NewsFetcherApi):
    def __init__(
            self, 
            news_fetcher: NewsFetcherApi = news
        ):
    ...
    def get_news_from(self, platform: Literal['cointime', 'caixin', 'jin10'], start: datetime) -> List[NewsInfo]:
        ...
        @with_lock(f'{platform}_query_lock', max_concurrent_access=1, expiration_time=300, timeout=300)
        def lock_part():
            with create_transaction() as db:
                ...
                if cache_time_range['query_start'] <= start_ts and start_ts < cache_time_range['query_end']:
                    ...
                    news_list_by_remote = self.news_fetcher.get_news_from(platform, ts_to_dt(cache_time_range['end']))
        ...
        def lock_part():
            with create_transaction() as db:
                ...
                if cache_time_range['query_start'] <= start_ts and start_ts < cache_time_range['query_end'] and cache_time_range['query_end'] <= end_ts:
                    logger.info(f"本地{platform}新闻缓存覆盖前一部分范围")
                    news_result = db.news_cache.get_news_by_time_range(platform, start, end)
                    news_list_by_remote = self.news_fetcher.get_news_during(platform, ts_to_dt(cache_time_range['end']), end)
                    map_by(news_list_by_remote, db.news_cache.add)
                ...
                if start_ts < cache_time_range['query_start'] < end_ts and end_ts < cache_time_range['query_end']:
                    ...
                    news_result = self.news_fetcher.get_news_during(platform, start, ts_to_dt(cache_time_range['start']))
                    news_list_by_local = db.news_cache.get_news_by_time_range(platform, start, end)
                ...
                if start_ts < cache_time_range['query_start'] and cache_time_range['query_end'] <= end_ts:
                    ...
                    news_list_by_local = db.news_cache.get_news_by_time_range(platform, start, end)
                    news_result_previous = self.news_fetcher.get_news_during(platform, start, ts_to_dt(cache_time_range['start']))
                    news_result_after = self.news_fetcher.get_news_during(platform, ts_to_dt(cache_time_range['end'] + 1), end)
                ...
                news_result = self.news_fetcher.get_news_during(platform, missed_start, missed_end)
                map_by(news_result, db.news_cache.add)
