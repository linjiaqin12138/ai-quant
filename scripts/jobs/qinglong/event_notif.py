...
if __name__ == '__main__':
    try:
        ...
        for symbol in interesting:
            history = OhlcvHistory(symbol, interval)
        ...
        if message:
            content = '\n'.join(message)
            for symbol in interesting:
                if content.find(symbol) >= 0:
                    res = long_short_ratio_info(symbol, interval, 1)
                    if res.status_code == 200:
                        long_short_info = res.json()[0]
                        log_info(f"{symbol} {interval} 多空比: {float(long_short_info['longAccount']) / float(long_short_info['shortAccount'])}")
                    else:
                        print(res)
            send_push({
