...
from lib.dao.data_query import get_all_pairs, get_ohclv
from lib.dao.event import get_event, set_event
from lib.dao.exchange import (
    buy_at_market_price,
    fetch_order,
    fetch_ticker,
    get_remain_money,
...
# Key is order_id
OrderContext = TypedDict("OrderContext", {"buy_price": float})
BuyTenSellFiveEventContext = Dict[str, Dict[str, OrderContext]]
GlobalContext = TypedDict(
    "GlobalContext",
    {
        "remain": float,
...
def add_order(
    events: BuyTenSellFiveEventContext, symbol: str, id: str, context: OrderContext
):
...
def action_decline_over(pair: str, decline_rate: float, spend: float):
    log_info(
        f"{pair} decline over {decline_rate}% in {interval_min}min: {round(decline_rate, 2)}%"
    )
    result = buy_at_market_price(
        pair,
        reason=ACTION_REASON,
        spend=spend if GLOBAL_CONTEXT["remain"] >= 20 else GLOBAL_CONTEXT["remain"],
    )
    ...
    log_info(
        f'buy {pair} with USDT {cost} and try to sell when price up from {result["average"]} to {to_sell_at_price}, gain money {to_gain} with rate {decline_rate / 2}'
    )
        ...
        for pair in pairs:
            ...
            for order_id in order_ids:
                ...
                if order_info["status"] != "open":
                    log_info(
                        f'{pair} 的在价格为{orders[order_id]["buy_price"]}买入，{order_info["average"]}卖出的单已经被卖出'
                    )
                    del orders[order_id]
                    # orders[order_id] = None # Will store null into json
                elif (
                    curr_ts() - order_info["timestamp"] / 1000
                ) / 84600 > 3 and is_eight_clock():
                    past_days = int(
                        (curr_ts() - order_info["timestamp"] / 1000) / 84600
                    )
                    ticker = fetch_ticker(pair)
                    curr_price = ticker["last"]
                    rate = (
                        (curr_price - orders[order_id]["buy_price"])
                        / orders[order_id]["buy_price"]
                        * 100
                    )
                    log_info(
                        f'{pair} 的在价格为{orders[order_id]["buy_price"]}买入，{order_info["price"]}卖出的单已经超过{past_days}天没有卖出, 现在卖出将{"盈利" if rate > 0 else "亏损"}{abs(round(rate, 4))}%, 当前价格为{curr_price}'
                    )
            if not orders:
