...
def turtle_trade(pair, frame = '1d', total_money = 100.0, max_round = 1, min_window = 20, max_window = 20, stop_decline_rate = None):
    logger.info(f'Run Turtle Trade Bot for pair {pair} with max_round {max_round}, min_window {min_window}, max_window {max_window}, stop_decline_rate {stop_decline_rate}')
    ...
    if context is None:
        ...
        if remain_money < total_money:
            logger.info(f'Remain money less than {total_money} start turtle trade for {pair}, abort')
            return
    ...
    df = get_ohclv(pair, context['frame'], limit=max(context['minWindow'], context['maxWindow']) + 2)
    assert df['timestamp'].iloc[-1].to_pydatetime() == unify_dt(datetime.datetime.now(), 86400)
    ...
    if df['close'].iloc[-2] > df['max_in_window'].iloc[-3] and context['holdMoney'] > 0.5 and (context['maxRound'] > context['roundNumber']):
        ...
        buy_result = buy_at_market_price(pair, spend=to_spend, reason=ACTION_REASON, amount=None)
        ...
        logger.info(f'Expected Spend {to_spend}, actual spend {actual_spend}, get amount coin {actual_gain_amount}')
