...
def loop():
    ...
    for i in range(1, len(df)):
        if df['position'].iloc[i] > 0 and current_usdt > 0:
            ...
            current_btc = buy
            logger.info(f"[{df['timestamp'].iloc[i]}] 花{current_usdt} USDT 买入 BTC {current_btc}，手续费 {cost} USDT")
            current_usdt = 0
            ...
            logger.info(f"[{df['timestamp'].iloc[i]}] 卖出 BTC {current_btc}，得到 USDT {current_usdt} 手续费 {cost} USDT 收益 {trade_gain} 收益率 {gain_rate}%")
            current_btc = 0
